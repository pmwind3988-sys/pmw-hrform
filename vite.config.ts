...
/**
 * `frame-src` includes `blob:` so the PDF preview dialog can frame a document
 * it fetched itself. Without it the frame is blocked outright and the dialog
 * shows a blank white panel with no error the user can see -- which reads as an
 * empty PDF rather than a blocked frame.
 *
 * It is a narrow allowance: a blob URL is same-origin and can only be minted by
 * this app's own scripts, so anything able to create one already has script
 * execution. `worker-src`, `img-src`, `media-src` and `connect-src` here all
 * permit `blob:` already. KEEP IN STEP WITH `vercel.json`, which carries the
 * production copy of this policy.
 */
const CSP = "default-src 'self'; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval'; worker-src 'self' blob:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; connect-src 'self' data: blob: https://*.sharepoint.com https://*.microsoftonline.com https://*.microsoft.com https://graph.microsoft.com wss://*.sharepoint.com; img-src 'self' data: blob: https:; media-src 'self' blob: data: https://*.sharepoint.com https://*.sharepointonline.com; frame-src 'self' blob: https://*.sharepoint.com https://*.officeapps.live.com https://*.office.com https://login.microsoftonline.com;"
