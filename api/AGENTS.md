...
| Public review link | `workflow-link.ts` | `POST /api/workflow-link` `{slug, responseItemId, layerNumber, revokeExisting?}`. Mints the signed link for one public layer of one submission, optionally revoking what was issued before. Scoped like `expand-group.ts`: the caller names a form + layer + submission, never a token, and the route refuses unless the layer is public and the submission is sitting on it. |
| Signed public grants | `_utils/publicGrant.ts` | Mint/verify `v1.<payload>.<HMAC>` links bound to one submission + layer + expiry, plus the `WorkflowGrantSerials` revocation helpers. Signed with `PUBLIC_LINK_SECRET` (falls back to `CRON_SECRET`) — **never** `API_SECRET_KEY`, which ships to the browser. |
| Declared identity | `_utils/publicIdentity.ts` | What a public link holder must state about themselves, and validation of what they stated. **Mirror of `src/utils/publicIdentity.ts` — keep both in sync.** |
