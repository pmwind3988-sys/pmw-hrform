...
- **Managed at `/admin/portal-accounts`** (`AdminPortalAccountsPage.tsx`), HR Forms Owner-only, reached from the header menu. `list-accounts` answers `provisioned: false` instead of erroring when the list is missing, which is what renders the "Set up" state on a fresh deployment; `ensure-schema` creates the accounts list **and** the access log together, because an accounts list without its log would issue accounts that silently record nothing.
