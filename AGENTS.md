...
- **What they are.** Anyone with a Google account. They sign themselves in, become a **permanent** member — there is no expiry column on the record at all — and land in the `Guest Members` list, one item per person, Title = the Google-verified address, lowercased.
- **This replaced HR-issued portal accounts** (login ID + password, generated once on a hand-over panel), which are deleted in full: the scrypt account hashing, the lockout counters, the enumeration-timing defences, the `portal-*` actions and the credential dialogs. Holders lost access on the deploy and sign in with Google instead. Their access-log history was left untouched. The `Internal Accounts` list is **not** deleted by the application — removing an HR records list is a decision made outside the software. `_utils/passwordHash.ts` is what survived, because the learning hub's content locks still store a password.
- **Nobody is refused at the door, and that is the whole design.** `guest-sign-in` admits any verified Google identity. What a member may *reach* is decided separately, in two stages: `profileComplete` (they have said who they are) and `learningApproved` (an HR Forms Owner said yes). Signing in is open; the library is not.
- **The Google token is never trusted from the browser.** `_utils/googleIdentity.ts` verifies it against Google's published JWKS — RS256 only (the algorithm is ours to fix, never the token's to nominate), issuer, `exp`/`iat` with 60s of skew, `email_verified`, and **`aud` against our own client ID**. That last one is the load-bearing check: a token issued for somebody else's Google app is validly signed and utterly useless as proof that its holder meant to sign in here. Keys are cached to Google's `max-age`, and a stale set is preferred to no sign-in.
- **`INTERNAL_SESSION_SECRET` must never be `API_SECRET_KEY`**, unchanged from the portal accounts. The latter ships to every browser as `VITE_API_SECRET_KEY`; signing sessions with a public value would let anyone holding the bundle mint a token for any address. Sign-in answers 503 below 32 characters rather than signing with something weak.
- **Sessions are stateless HMAC tokens** (`pmwg1.<base64url payload>.<sig>`), carrying the member's `tokenVersion`. The prefix changed from `pmwp1` deliberately: a token minted by the deleted password system must fail at the door, not resolve to a person. A disable or an approval change bumps that version and retires every token already issued. `isGuestSessionCurrent` enforces it and caches member state 60s, so a revocation takes effect within about a minute rather than instantly.
- **`Title` is indexed at provisioning, and there is no scan fallback.** This is the one place the portal-account code could not simply be renamed. It coped with SharePoint's ~5,000-item filter threshold by scanning up to 2,000 items, under a comment reading *"this is not a list that grows with use"* — true for hand-issued accounts, false the moment sign-up is public. Past a couple of thousand members that fallback cannot cover its own list, and a real member is intermittently told they do not exist. `ensureFieldIndexedViaSPRest` removes the cause; the scan is gone because a scan that cannot cover the list turns "SharePoint is unreachable" into "you have no account".
- **Guest members reach `/member`, `/learning` (once approved) and the public routes, nothing else.** `App.tsx` renders a **separate route table** for `pageState === "member"` rather than threading a flag through the main one — an allowlist cannot leak a route added later, which twenty individually-guarded routes eventually would. The profile form is rendered *instead of* that table, not as a route inside it, so no path reaches anything while it is outstanding.
- **`acquireLearningIdentityToken` and `acquireCareerPortalToken` are the client-side choke points**, unchanged in shape: both return the stored guest token when there is one, before touching MSAL, so every learning and careers page works for guest members with no change of its own.
- **`resolveLearnerViewer` returns a status, not `Learner | null`.** A guest can fail to be a learner three different ways — sign in, finish your profile, wait for HR — and each needs a different thing said to them. Collapsing them into `null` sent a member who was merely awaiting approval back to a sign-in screen they had already used, which reads as the app being broken. The three codes are `learning-sign-in-required`, `guest-profile-incomplete`, `guest-awaiting-approval`.
- **View rows namespace guests apart** via `guestViewerKey` (`learning-views|guest|{email}`), so somebody who is both a staff member and a self-registered guest at the same address counts as two viewers rather than silently merging.
- **Every guest route is an `action` on `/api/learning-materials`**, taking over the slots the `portal-*` actions vacated — `guest-sign-in`, `guest-me`, `guest-save-profile`, `guest-departments`, `guest-my-submissions`, and the admin `guest-*` set. Vercel's Hobby plan caps a deployment at 12 serverless functions, `api/` is at exactly 12, and the limit bites *after* a successful build with nothing in the build log. `api/_utils/deploymentLimits.test.ts` fails the suite instead.
- **Managed at `/admin/guest-members`** (`AdminGuestMembersPage.tsx`), HR Forms Owner-only. It is an approvals queue, not a credential factory: approve or revoke learning, disable or re-enable an account. Nothing on it creates a member — members create themselves. **Paged with a search box**, because the list grows on its own; `guest-list-members` answers `provisioned: false` instead of erroring when the list is missing, which renders the "Set up" state.
- **Approval is withheld until the profile is complete.** Granting the library to a row with no name and no department defeats the point of the log it will be written into, so the switch is disabled until then.
- **The department dropdown reads the `Department Approver Directory`** HR already maintains, so a member's department is the same string the approval routing understands. That directory lists *PMW's own* departments — a genuinely external contractor may find nothing that fits and pick the nearest wrong one. An "Other, please specify" escape hatch is the fix if that becomes common; it is not built. An unreadable directory falls back to a free-text field rather than blocking sign-up.
- **`guest-my-submissions` walks the form registry**, because every HR form writes to a response list of its own and there is no single table of submissions to filter. One read per form, capped, on a member's own request only. **History starts at launch**: public form submissions used to record the literal text `GUEST` as the submitter, with nothing tying them to a person, so earlier submissions cannot appear and never will. Job applications are unaffected — they already store the applicant's address.
- **`submit-form` stamps a guest member's own address over "GUEST"** — from the session token in the `Authorization` header, never from the request body, because that endpoint is public and a body field naming the submitter would let anybody file under somebody else's name. It is stamped **after** `applyLayerConfigWorkflow`, and that ordering is load-bearing: routing treats "GUEST" as unusable and parks a layer rather than guessing at an approver, so setting a real address first would start resolving chain layers against a directory that has never heard of these people, and would email a contractor as though they were staff. Routing keeps today's behaviour; only the stored row learns who sent it.

### Named access log (guest members only)

- **`Learning Access Log`, append-only**, one row per view: Title = the member's Google address, plus `ViewerName`, `ViewerPosition`, `ViewerDepartment`, `MaterialId`, `MaterialName`, `ViewedAt`. Nothing in `learningAccessLog.ts` updates or deletes a row — an audit trail editable from the screen that displays it is not evidence.
- **M365 staff are never written here.** Their views stay behind the one-way hash that only answers "how many distinct people". The split is the product decision, not an oversight — see `PDPA_COMPLIANCE.md`. Folding the two lists together would quietly extend named tracking to every employee.
- **Name, position and department are stamped into each row, not looked up on read.** A guest member may edit their own profile. If the log showed current values it would be rewritable by the person it is about — change your job title and the record of what you opened last year changes with it.
...
- **The notice moved into the product.** A portal account holder was told about this log out loud, at hand-over. A member who signed themselves up has had no such conversation, so `GuestMemberPage` states it where learning access is granted. Act 709's notice-and-choice principle is why this is a requirement of the feature rather than a nicety.
- **Retention is an open question.** The trail never shrinks and now covers a population that grows on its own. How long rows should be kept is a records-keeping decision, not a technical one.
...
- **A library can be ready while its tracking list is not.** `libraryReady` reports only the document library, so a site whose `Learning Material Views` list was never created serves every material perfectly while counting nothing — and the admin screen offered no way to fix it, because the provisioning button only appeared when the *library* was missing. `list` now also returns `viewsReady` (free: it comes off the `readViewIndex` the handler already does), and `recordView` returns 0 rather than throwing when the list is absent. That last part matters beyond the count: the named access log is written *after* the count, so an unprovisioned counter was silently costing portal accounts their audit trail too.
...
- **Password locks (topics and single materials)** — `api/_utils/learningLocks.ts`, managed from `/admin/learning` by the same switch pattern as Download.
  - **A lock hides rather than merely refuses.** A locked topic's materials, subtopics and cover thumbnails are not in the `list` response *at all* — not searchable, not linkable, not counted. A locked material comes back with `thumbnailUrl`, `mediaUrl` and `description` stripped. "Blocked from preview" is the requirement, and a card autoplaying eight seconds of a video it then asks a password for would not meet it. Filtering client-side would have shipped it all to the browser and asked it not to look.
  - **The nearest lock wins.** A material's gate is its own password if it has one, else the closest locked folder above it (`materialLock`). Nobody is ever asked for two passwords to reach one file. `LearningMaterial.lockOwn` distinguishes "has its own password" from "inherited from a topic" — the admin switch reads `lockOwn`, because there is nothing to turn off on an inherited one.
  - **Nothing is persisted.** Unlocking issues a short-lived signed pass (`pmwl1.<payload>.<sig>`) that the browser holds in **React state only** — never `localStorage`, `sessionStorage` or a cookie. A reload starts locked again. A topic pass covers browsing that topic for the visit; a material pass is spent on one open and discarded by `leaveMaterial()` when the viewer closes or steps to the next image.
  - **Passes are bound three ways**: to the viewer key, to the scope+target, and to a fingerprint of the stored hash — so changing or removing a password retires every pass already issued, immediately.
  - **The caller names the material or the topic, never the lock.** `unlock` resolves the effective lock server-side from the item's own `parentReference` (`driveItemFolderPath`), read back off SharePoint. Letting a request nominate which lock to check against is how a per-material password would get used to open a department's folder.
  - **`record-view` is gated too**, not just `open-material`. The named access log is evidence: a row saying somebody opened a locked briefing has to mean they got through its password, not that they knew the drive item id.
  - **Passwords are scrypt**, the same parameters and stored format as a portal account, in the same `learning-materials-settings` JSON blob. There is no read path — admins replace, never look up. `parseMaterialSettingsInput` deliberately strips `passwordHash` so `update-material` can never install or clear one from a request body, and `publicMaterialSettings` keeps the hash out of admin responses.
  - **Signed with `INTERNAL_SESSION_SECRET`, falling back to `SYSTEM_CLIENT_SECRET`** via a derived subkey (so a pass is never confusable with a portal session token). No new environment variable: nothing on this route answers at all without `SYSTEM_CLIENT_SECRET`.
  - **Admins read the library through `admin-list`**, not `list` — same HR Forms Owner gate as every other library edit. An admin sets these passwords; a management screen that hid the folder whose password they had just set, with no way back short of typing it, would be unusable.
  - **Guess throttling is best-effort and per (viewer, lock)**, in memory. Serverless instances come and go, so it is not the durable counter a portal account's lockout is — and that is deliberate: one person guessing must never shut a shared password out for the department it was handed to. The real cost to a guesser is scrypt.
