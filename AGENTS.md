...
- **All columns are single-line text**, including numbers and timestamps. `ensureTextFieldViaSPRest` (FieldTypeKind 2, delegated token) is the only column-creating path this tenant permits — the app-only principal is refused, exactly as in `ensureCareerPortalAccessSchema`.
- **Provisioning runs entirely as the admin, list included.** The first attempt created the list app-only and only the columns delegated; SharePoint answered `403 accessDenied` on `POST /sites/{id}/lists` and the "Set up" button failed on a site that did not already have the list. `ensureListViaSPRest` fixed it. The same refusal applies to every list and library in this repo, so treat any `ensureGenericList`/`ensureDocLibrary` call on a *creation* path as a latent bug — see the app-only note in `api/AGENTS.md`.
- **Lookups by login ID cannot rely on a Graph `$filter` alone.** `Title` is unindexed on a list SharePoint has just created, and Graph answers `$filter` on an unindexed column with a 400 — so the first account ever issued could be created and then neither signed into nor reset. `queryListItemByFields` now always sends `Prefer: HonorNonIndexedQueriesWarningMayFailRandomly`, and `readAccountRow` falls back to scanning when the filtered read *errors* (never when it merely returns nothing — scanning on a miss would make an unknown login ID answer slower than a real one).
