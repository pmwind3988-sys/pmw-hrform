...
/*
 * The prefix rule and the sign-in access gate used to live here too. They now
 * live on the server — `routePrefixAllowsLayerType` in
 * `api/_utils/workflowLink.ts`, the assignment check in `api/evaluate.ts` —
 * because the reviewer page no longer decides what it may show. Keeping a
 * second copy here would be two answers to "what may this person see", which
