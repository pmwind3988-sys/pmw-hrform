...
/**
 * Mirrors DEFAULT_PUBLISH_KEY in formBuilderSP.ts. Declared locally so this
 * module stays dependency-free and does not pull the builder into the bundle —
 * the same reason submissionFilters.ts keeps its own copy.
 */
const DEFAULT_PUBLISH_KEY = "production";

/**
 * Key for one published survey snapshot. A form version can carry several
 * publish profiles, each with its own schema, so version alone is ambiguous.
 * "::" is a safe separator because publish keys are slugified before storage,
 * so a colon can never appear inside one.
 */
export function surveySnapshotKey(formVersion: string, publishKey: string): string {
...
export async function loadConfig(
  ...
  try {
    ...
    for (const item of versionItems) {
      ...
      const publishKey = String(item.PublishKey || "");
      ...
      // One version can carry several publish profiles, each with its own
      // survey snapshot. Keying by version alone let the last row read win, so
      // a submission could render against another profile's schema.
      surveyJsonByFormVersion[formTitle][surveySnapshotKey(formVersion, publishKey)] = parsedSurveyJson;
      // Version-only fallback, for submissions written before PublishKey was
      // recorded. The default profile wins it outright; otherwise first row
      // seen holds it, rather than whichever happened to be read last.
      const isDefaultProfile = !publishKey || publishKey === DEFAULT_PUBLISH_KEY;
      if (isDefaultProfile || !(formVersion in surveyJsonByFormVersion[formTitle])) {
